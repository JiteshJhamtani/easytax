    ...
    public function data(Request $request)
    {
        ...
        return datatables()->of($query)
            ...
            ->addColumn('ack_no', function($a) {
                ...
                if ($ackMedia) {
                    // NOTE: Make sure you have a route named 'agent.documents.download' or swap it with your actual agent download route!
            ...
            // 3. BALANCE SHEET BUTTON
            ->addColumn('balance_sheet', function($a) {
                if ($a->service->slug !== 'itr-filing') return '-';
                $url = route('agent.applications.balance-sheet', $a->id);
                return '<a href="'.$url.'" class="btn btn-sm btn-outline-success font-weight-bold" style="white-space: nowrap;">
                            <i class="fas fa-file-excel mr-1"></i> Generate Now
                        </a>';
            })
    ...
    */
    public function balanceSheetForm($id)
    {
        ...
        foreach ($application->media as $media) {
            ...
            // Only try to read it if it is actually a PDF file
            $isPdf = strtolower(pathinfo($media->file_name, PATHINFO_EXTENSION)) === 'pdf' || $media->mime_type === 'application/pdf';
        ...
        // 4. Send everything to the Agent view
